package com.liferay.captcha.simplecaptcha.override;
...
public class CustomSimpleCaptchaImpl implements com.liferay.portal.kernel.captcha.Captcha {
    ...
    private Object _getInstance(String className) {
        int parameterToBePassed = 6;
        if (className.toLowerCase().contains("textproducer")) {
            _log.error(" className " + className);

            if (className.contains(";")) {
                parameterToBePassed = Integer.parseInt(className.split(";")[1]);
        ...
        try {
            ...
            if (className.toLowerCase().contains("textproducer")) {
                ...
                try {
                    ...
                    instance = paramsConstructor.newInstance(parameterToBePassed);
                } catch (Exception e) {
